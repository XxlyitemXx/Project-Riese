...
class WarnCog(commands.Cog):
    ...
    async def warn_add(
        ...
        cursor = self.conn.cursor()
        cursor.execute(
            "SELECT warn_count FROM warns WHERE guild_id = ? AND user_id = ?",
            (guild_id, user_id),
        )
        row = cursor.fetchone()
        ...
        await interaction.response.send_message(
            f"{member.mention} has been warned. They now have {warn_count} warning(s). Reason: {reason}"
        )
    ...
    async def apply_punishment(
        ...
        if warn_count == 2:
            await member.timeout(
                datetime.timedelta(minutes=30), reason="Reached 2 warnings"
            )
            await interaction.channel.send(
                f"{member.mention} has been timed out for 30 minutes due to reaching 2 warnings."
            )
        elif warn_count == 4:
            await member.timeout(
                datetime.timedelta(hours=6), reason="Reached 4 warnings"
            )
            await interaction.channel.send(
                f"{member.mention} has been timed out for 6 hours due to reaching 4 warnings."
            )
        elif warn_count == 5:
            await member.timeout(
                datetime.timedelta(hours=24), reason="Reached 5 warnings"
            )
            await interaction.channel.send(
                f"{member.mention} has been timed out for 24 hours due to reaching 5 warnings."
            )
        elif warn_count >= 6:
            await member.kick(reason="Reached 6 warnings")
            await interaction.channel.send(
                f"{member.mention} has been kicked from the server due to reaching 6 warnings."
            )
    ...
    async def warn_remove(
        ...
        await interaction.response.send_message(
            f"Removed a warning from {member.mention}. They now have {warn_count} warning(s).",
            ephemeral=True,
        )
    ...
    async def warn_list(self, interaction: nextcord.Interaction):
        ...
        cursor = self.conn.cursor()
        cursor.execute(
            "SELECT user_id, warn_count FROM warns WHERE guild_id = ?", (guild_id,)
        )
        rows = cursor.fetchall()

        if not rows:
            await interaction.response.send_message(
                "There are no warned users in this server.", ephemeral=True
            )
            return
